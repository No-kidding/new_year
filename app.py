...
if st.session_state.stage == 1:
    ...
    st.caption(f"放心，不是真的拷问——就是几个灵魂小问题，答完你会更了解自己（也可能更迷茫，但没关系）。")
    st.markdown("---")
...
if st.session_state.stage == 2:
    if not st.session_state.quiz_done:
        ...
        st.caption(f"三支签，三个维度，一个都不能少。友情提示：这里没有下下签，放心大胆抽，手气差也差不到哪去。")
        st.markdown("---")
...
if st.session_state.stage == 3:
    if not st.session_state.sign_done:
        ...
        st.caption(f"前方高能预警：一大波走心文字正在靠近。建议准备好纸巾（感动用的，不是伤心用的）。")
        st.markdown("---")
